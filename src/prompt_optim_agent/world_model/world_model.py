...
class WorldModel(Generic[State, Action]):
    ...
    def eval_instruction_with_loader(self, task, eval_prompt, dataloader, record_outputs=True):
        ...
        self.logger.info(f'task_name: {task}')
        self.logger.info(f'eval_prompt: {eval_prompt}\n')
        self.logger.info(f'prompt example: \n{build_forward_prompts_func(["example_question"], eval_prompt)[0]}\n')
        ...
        for batch in pbar:
            batch_prompts = build_forward_prompts_func(batch['question'], eval_prompt) #prompt + question
            responses = batch_forward_func(batch_prompts) #ex) 'To find the name of the 60 cm tall penguin, we need to look at the table \n\nFrom the table, we can see that Vincent 60 cm.\n\n<answer>(C) Vincent</answer>'
            preds = task.batch_clean_responses(responses) #ex) ['C']
            labels = task.clean_labels(batch['answer']) #ex) ['C']
            ...
            if not isinstance(metric, tuple):
                ...
                self.logger.info(f"Test Metric: {metric:.4f}")
        ...
        metric = task.cal_metric(all_preds, all_labels, all_questions)
        self.logger.info(f"eval_output : {eval_output}")
        return metric, eval_output
